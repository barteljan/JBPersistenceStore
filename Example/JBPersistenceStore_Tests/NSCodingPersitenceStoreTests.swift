...
class NSCodingPersitenceStoreTests: XCTestCase {
    ...
    func testVersionChangedHandlerDoesTriggerOnForgottenVersion() {
        ...
        _ = NSCodingPersistenceStore(databaseFilename: uuid)
        self.forgetVersion(ofDatabaseFilename: uuid)

        _ = NSCodingPersistenceStore(databaseFilename: uuid, version: expectedNewVersion) { (oldVersion: Int, newVersion: Int) in
    ...
    func testVersion() {
        let uuid = NSUUID().uuidString
        let codingStore = NSCodingPersistenceStore(databaseFilename: uuid, version: 2) { (_: Int, _: Int) -> Void in }
    ...
    func testAsynchVersionChangedHandlerHasToCallSuccesToSeeVersionchange() {
        ...
        let oldVersion = 0
        let newVersion = 1
        _ = NSCodingPersistenceStore(databaseFilename: uuid, version: oldVersion)
    ...
    func testVersionChangeHandlerDoesntTriggerOnDatabaseCreation() {
        let uuid = UUID().uuidString
        _ = NSCodingPersistenceStore(databaseFilename: uuid, version: 2) {
            (_: Int, _: Int) in
    ...
    func testVersionChangedHandlerDoesTriggerOnDatabaseVersionChange() {
        let oldVersion = 2
        let newVersion = 3
        let dbname = NSUUID().uuidString
        let exp = expectation(description: "wait for versionchangehandler")

        _ = NSCodingPersistenceStore(databaseFilename: dbname, version: oldVersion) { (_: Int, _: Int) -> Void in }
    ...
    func testVersionChangedHandlerDoesNotTriggerOnStableDatabaseVersion() {
        ...
        _ = NSCodingPersistenceStore(databaseFilename: uuid, version: version) {
            (_: Int, _: Int) in
            XCTFail("Should not fire on creation")
        }
        _ = NSCodingPersistenceStore(databaseFilename: uuid, version: version) {
    ...
    func testAsyncGetByIdentifier() {
        do {
            ...
            try store.get("666") { (item: TestPersistable?) in
    ...
    func testAsyncGetByIdentifierAndType() {
        do {
            ...
            try store.get("666", type: TestPersistable.self) { (item: TestPersistable?) in
                XCTAssertNotNil(item)
                XCTAssert(item!.title == "Testtitel")
                expect.fulfill()
            }
    ...
    func testAsyncGetAllByType() {
        do {
            ...
            try store.getAll(TestPersistable.self) { (items: [TestPersistable]) in
    ...
    func testAsyncExists() {
        do {
            ...
            try store.exists(persistable) { (exists: Bool) in
                XCTAssertTrue(exists)
                expect.fulfill()
            }
    ...
    func addView(store: NSCodingPersistenceStore) throws {
        do {
            try store.addView("TestPersistablesByIdType",
                              groupingBlock: { (_: String, _: String,
                  ...
                  _: String,
                  key2: String,
                  _: TestPersistable) -> ComparisonResult in
    ...
    func testAsyncGetAllFromView() {
        do {
            ...
            try store.getAll("TestPersistablesByIdType") { (items: [TestPersistable]) in
                XCTAssert(items.count == 3)
                expect.fulfill()
            }
